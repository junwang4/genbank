...
function update_taxid_to_Annotation($mysqli) 
{
    ...
    while ($row = $result->fetch_object()) {
        ...
        if ($organism_tax[$organismId] != NULL) {
            ...
            $q = "UPDATE Annotation SET tax_id = " . $tax_id . " WHERE accession = " . $accession;
            if (! $mysqli->query($q) ) {
...
function load_csv_files($mysqli)
{
    ...
    foreach ($tables as $table)
    {
        ...
        if ($table == "Annotation")
        {
            $null_clause = "(accession, version, @keywordsId, sourceId, organismId, @commentId, @dblinkId, locus, definition, @segment, locus_name, locus_sequence_length, @locus_sequence_strands, locus_nucleic_acid_type, locus_linear_circular, locus_division_code, @locus_date, @tax_id) SET keywordsId = nullif(@keywordsId,''), commentId = nullif(@commentId,''), dblinkId = nullif(@dblinkId,''), segment = nullif(@segment,''), locus_sequence_strands = nullif(@locus_sequence_strands,''), locus_date = STR_TO_DATE(@locus_date, '%d-%b-%Y'), tax_id = nullif(@tax_id,'')";
        }
...
function create_table($mysqli, $table) 
{
    ...
    if ($table == "Reference") {
        ...
        $sql = "CREATE TABLE IF NOT EXISTS " . $table . " (
            ...
            version INT,
            keywordsId VARCHAR(50),
            sourceId VARCHAR(50),
            organismId VARCHAR(50),
